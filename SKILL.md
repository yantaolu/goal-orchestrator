...
- `concise`：仅当用户明确要概要、快速建议、简要判断、一页结论或同等压缩交付时使用。压缩的是展开，不是决策闭环：保留推荐与关键机制或理由、至少一个适用边界或升级条件，以及一个最关键验证门；它们可以自然合在一两段里，不必凑固定标题、数字、完整数据模型、接口或排期。
...
比较不是一律附带双原型；先看推荐是否真正取决于运行表现、兼容性或体验差异。

- 若需要验证这类差异，让两种候选使用同一代表性真实数据集、同一目标设备或系统、同一任务和预算。候选轴是要比较的两种方案（例如 Electron 与 Tauri）；Windows/macOS 是平台轴，只是两种候选都要覆盖的验证环境，不能替代候选对照。
- 一旦选择受控双原型，两种候选都要在最终选型前完成最小可比垂直切片。计划逐项列出候选 A 与候选 B 各自完成同一端到端任务，并在两者结果齐备后才应用阈值；只验证推荐候选、把另一候选推迟到失败后，不算双候选对照。
- 在原型前写清可判定的度量和保持或切换阈值。用户没有给数值时，不虚构数字：用待填预算变量或硬门（如 `T_start`、`M_max`）表达。当前候选全部硬门通过则保持；只有当前候选至少一项硬门失败、另一候选在相同条件下通过且没有新增关键兼容或维护阻碍时才切换。
- 本轮没有真实完成双候选对照时，只能给基于现有信息的初步或倾向性推荐，不得写成已经证实的最终选型。若双原型成本不成比例，也诚实保留初步结论并说明后续证据门。
- 纯成本、政策、合规或其他无法由运行原型回答的比较，直接用相应证据、约束和决策门判断，不强行制造双原型。
...
3. 创建且仅创建一个协调者 `R0`。在用户可见文本第一次提到时，自然写成“目标实现协调者（R0）”或“协调负责人（R0，唯一）”，让读者一眼知道协调责任归属；后文继续使用正常职称即可，不必机械附角色表。它负责路由、质量门、返工和汇总，不替代领域生产者或验证者。
4. 从结果、能力和适用生命周期推导角色。关键成果需要独立验证时，验证者应不同于该成果的设计者和实现者，`R0` 也不担任独立验证者；这样质量门才不是自我确认。覆盖真实所需的交付、运行和反馈职责。
...
2. **深度匹配：** 普通方案是否至少达到 `standard` 完整度；只有明确压缩请求才为 `concise`，且仍保留推荐、边界与关键验证门；`deep` 是否补足真正相关的机制和非功能约束。
3. **内容实质：** 是否给出了领域具体内容，而不是用流程、角色或套话代替答案。
4. **组织适配：** 是否使用了最轻且足够的组织深度；完整组织是否让唯一 `R0` 身份自然可识别，并保持设计、实现与独立验证的边界。
...
8. **比较验证：** 是否把候选轴与平台轴分开；若最终选型依赖受控双原型，两个候选是否都已逐项完成同一端到端切片，并在两者结果齐备后应用阈值。若只验证一个候选，就不能给最终选型，应保留为初步结论并说明后续证据门。
