...
仅当组织深度为 `simulated_team` 时使用本协议。它负责虚拟生产者、验证者与整合者怎样协作形成和审查方案，不是目标规划与情景推演的全部协议，也不能把模拟组织本身变成交付物的主角。

没有选择 `simulated_team` 时不执行本协作协议，但仍可依据 [goal-planning-protocol.md](goal-planning-protocol.md) 深入规划目标、推演真实组织或运行场景。普通方案、架构比较、简单目标和责任映射不需要为了本协议补团队、审查记录、可行性标签或边界声明。
...
进入前必须已经完成意图构造，并判断用户要的是具体方案、目标实现，还是既有方案实施。本协议的协作审查服务于该意图：

- 方案请求：先形成具体方案，再让组织审查高风险或跨域部分；实施仍然可以保持简要。
- 目标请求：先执行目标规划与情景推演协议；虚拟组织只在跨专业生产、交接或独立验证确有价值时继续完善方案和实施路径。
...
根据问题合并或省略不适用的轮次，但保留真正需要的生产、独立验证、整合与交付责任。这里模拟的是生成方案的团队协作；目标路径和真实组织运行的前向场景由目标规划协议负责，两者可以交换发现但不要混为一套角色剧情。
...
- **推演概念混淆：** 把真实组织运行场景写成虚拟团队对话，或因为没有虚拟团队就跳过目标情景推演。

## 完整性检查

- [ ] 已先构造可规划意图并完成所需的目标情景推演，再决定是否开始虚拟团队协作。
