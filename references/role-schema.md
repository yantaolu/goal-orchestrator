...
规范角色结构见 [role.schema.json](role.schema.json)，示例见 [sample-role.json](sample-role.json)。两者保持 `schema_version: "3.0"`；根结构化输出升级到 3.3 不改变角色契约。
...
- 用户明确要求用于生成和审查方案的完整虚拟团队或团队协作推演。若“组织设计”指事业部、岗位或管理层级等真实组织方案，本身不触发本角色协议。
...
3. 创建唯一协调者 `R0`，只赋予协调和决策路由能力。用户可见文本第一次提及时，自然写成“目标实现协调者（R0）”或“协调负责人（R0，唯一）”；之后保留正常职称，避免角色编号淹没方案。
...
追求最小但完整，而不是角色越多越好。一个边界清楚的负责人可以覆盖相邻阶段；关键成果的独立验证者不能同时是该成果的设计者或实现者，`R0` 也不担任独立验证者。
...
4. 每项主要成果有明确生产者和消费者。
5. 风险重要的关键成果有不同于设计者和实现者的独立验证者；`R0` 不担任独立验证者。
6. 角色名称不同不代表独立；若同一责任同时设计、实现并验证，仍视为缺口。
...
- 关键成果的设计、实现与独立验证是否在风险需要时分离，且验证者不是 `R0`？
- 是否只展示影响方案或实施的组织信息？
- 是否准确保留模拟与现实证据边界？
